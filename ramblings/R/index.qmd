...
A worthy collection of short R commands and tricks. 👍 *As featured in R Weekly [2023-W44](https://rweekly.org/2023-W44.html)*
...
ggplot(data = data, aes(x = Month, y = Production_Litres, colour = Region, linetype = Year, group = interaction(Region, Year))) +
  ...
  labs(title = "Australian Dairy Milk Production",
       subtitle = paste0(
         "<span style='color:#000000;'>**National Production**</span> and key dairy states ",
...
The cut_short_scale() function from the [{scales}](https://scales.r-lib.org/) package will conveniently convert your unclear labels to a more appropriate format. Lets apply it to our example from above. 🥳🎉
...
ggplot(data = data, aes(x = Month, y = Production_Litres, colour = Region, linetype = Year, group = interaction(Region, Year))) +
  geom_smooth(se=FALSE) +
  scale_color_manual(values = c("#000000","#428bca","#b5b682")) +
  scale_y_continuous(labels = label_number(scale_cut = cut_short_scale()), breaks=seq(0,900000000,100000000)) +
  labs(title = "Australian Dairy Milk Production",
       subtitle = paste0(
         "<span style='color:#000000;'>**National Production**</span> and key dairy states ",
         "<span style='color:#b5b682;'>**Victoria**</span> and ",
...
Nevertheless, you can avoid all this by combining the reduce function from the 🐈‍⬛ [purrr](https://purrr.tidyverse.org/) package with the relevant join command. Below is a simple example. 👇
...
Before diving into a loop of calculations, it's often beneficial to initialise an empty tibble to seamlessly store results from each iteration. Instead of manually specifying column names, you can streamline the process by leveraging the power of the [map_dfc()](https://purrr.tidyverse.org/reference/map.html) command in [{purrr}](https://purrr.tidyverse.org/) 🐈 Simply pass a vector of column names to tibble, and watch your data structure effortlessly take shape.
...
`map_dfc()` has now been superseded 🏗 by `map()` and I have also updated the code to be more explicate and inline with {purrr} recommendations. Thanks [Jim Gardner](https://jimgar.github.io/) for the [suggestions and edits](https://fosstodon.org/@jimgar/111409203326035011).
...
There was a time where I would export my R plots as pdfs and then convert them to .tiff .png or .jpeg with GIMP to get achieve that high quality finish 🖼️ That was until I came across [camcorder](https://github.com/thebioengineer/camcorder). camcorder simplifies the whole process of exporting and saving R plots and they come out looking fantastic 👌
...
Which exports a .png that looks like 👇 ![](images/2024_05_11_02_12_25.079715.png)

## Simulated multiple correlated normal distributions

Ever needed to generate multiple vectors with a complex set of correlations. This is R so of course there is a package for that. Queue [faux](https://debruine.github.io/faux/) and the [rnorm_multi()](https://debruine.github.io/faux/reference/rnorm_multi.htmlhttps://debruine.github.io/faux/reference/rnorm_multi.html) function. With this function you can define the number of values, means and standard deviation of each vector of variables generated, and of course the correlation among the variables. Check out the example below 👇
...
Sometimes you might want to direct your audience to specific datapoints in our plots. You could do this by adding an annotation arrow and text as described in an [earlier ramblings](https://lpembleton.rbind.io/ramblings/R/#highlight-points-of-interest-with-annotations), however if it is a cohort a data this might not be suitable. Instead a better approach might be to circle the relevant datapoints with shapes like circles ⭕ You can achieve in ggplot by essentially adding additional points with `geom_point()` but on a filtered subset of the data. Here is an example 👇
...
ggplot(data = precip_tbl, aes(x = precipitation)) +
  geom_density(colour = "#023047", fill = "#8ecae6") +
  geom_vline(xintercept = 34.4, colour = "red", size = 1.5) +
  ylab("Density") +
  xlab("Precipitation") +
  ggtitle("Annual Precipitation in US Cities") +
  theme_classic()
